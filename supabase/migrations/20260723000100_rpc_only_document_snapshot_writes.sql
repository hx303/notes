...
BEGIN
  ...
  -- DELETE and the five lifecycle UPDATE columns remain available to the
  -- authenticated owner after this migration. Pin the complete documents RLS
  -- policy set before the first ACL mutation so a same-name policy drift cannot
  -- widen those retained capabilities. The fingerprint was produced by a clean
  -- PostgreSQL 17 replay of 20260718000100 with this explicit search path.
  ...
  IF (
    ...
    RAISE EXCEPTION
      'documents owner RLS policy set or expressions drifted; retained lifecycle writes would be unsafe'
      USING ERRCODE = 'object_not_in_prerequisite_state';
  END IF;

-- Production-safe, read-only concurrency gate for 20260722000100.
-- The runbook accepts only the pass sentinel from this single statement.
...
SELECT
  'site_owner_role_invariant_activity_gate_passed' AS result,
...
UNION ALL
SELECT
  'site_owner_role_invariant_activity_gate_failed' AS result,
  300::INTEGER AS max_transaction_age_seconds
FROM activity_risk
CROSS JOIN lock_risk

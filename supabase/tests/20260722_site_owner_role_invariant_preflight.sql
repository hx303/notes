-- Production-safe, read-only preflight for 20260722000100.
-- Run as the linked database owner before the backup-verified migration push.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE procedure.oid = grant_rpc
      ...
      )) = '9760c14500ea0d408bb1d6439fef7886'
      AND procedure.prosrc NOT LIKE
        '%IF public.is_site_owner(target_uid) THEN%'
  ) THEN
    RAISE EXCEPTION
      'grant_role production baseline drifted or 20260722000100 is already present';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'is_site_owner security contract drifted';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'grant_role execute ACL baseline drifted';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'is_site_owner execute ACL baseline drifted';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'protected site-owner row or admin role drifted';
  END IF;
  ...
  IF actual_ledger IS DISTINCT FROM expected_ledger
    OR (SELECT count(*) FROM supabase_migrations.schema_migrations) <> 18
    OR (
...
SELECT
  ...
  'site_owner_role_invariant_preflight_passed' AS result
FROM public.site_owners site_owner

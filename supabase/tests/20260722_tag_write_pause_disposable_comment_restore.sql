-- Disposable-only restoration of the one intentionally drifted trigger comment.
...
DO $tag_write_pause_disposable_comment_restore_preflight$
BEGIN
     ...
     OR (SELECT count(*) FROM pg_catalog.pg_trigger trigger
         WHERE trigger.tgrelid = 'public.tags'::regclass
           ...
           AND pg_catalog.obj_description(trigger.oid, 'pg_trigger') =
             'wouldkeep disposable tag-write pause comment drift') <> 1 THEN
    RAISE EXCEPTION USING ERRCODE = '55000', MESSAGE = 'wouldkeep_tag_write_pause_disposable_comment_restore_preflight_failed';
  END IF;
END;
$tag_write_pause_disposable_comment_restore_preflight$;
...
SELECT 'tag_write_pause_disposable_comment_restored' AS result;

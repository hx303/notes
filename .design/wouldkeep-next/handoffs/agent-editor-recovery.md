...
- UI evidence (viewport, theme, state, screenshot path/diff): independent signed-in Draft-preview acceptance opened the detailed editor and observed it remain active for at least 35 seconds, passing the write-surface auth-refresh regression. Public responsive checks at 1,313, 1,024, and 390 CSS pixels found no page-level horizontal overflow; desktop sidebar scrolling and medium navigation behavior were present. A later signed-in branch-preview run created an explicitly private Codex test draft, observed `云端草稿已保存` plus a bound document URL, reloaded that URL, and observed `已加载云端草稿` with the exact title/body, private-default copy, and version history intact. A double-tab conflict attempt was safely denied when related-data loading remained incomplete; neither temporary divergent body became a new cloud version, the second tab was closed, and reloading the first restored the accepted cloud body. Static keyboard review confirmed the save action is in the natural focus order and hidden conflict controls are absent from the effective sequence. Offline storage, completed conflict actions, actual step-by-step keyboard traversal, and real sidebar overflow remain open.
...
- Next task prerequisites: wait for the `b9b7558e` preview deployment, then continue deployed-preview offline/conflict/retry and keyboard evidence when Chrome control and related-data reads are stable. Keep PR #26 Draft; do not mark Ready or merge without the remaining gate and explicit user approval.

...
- P05 editor recovery started from `a4662b1d`. The final local checkpoint adds a wired IndexedDB outbox, lock-scoped interrupted-save recovery, latest-intent coalescing, authoritative durable acknowledgements, Web Locks/BroadcastChannel coordination, inline three-choice conflict handling, account/document latest-wins guards, a safe document-switch loading gate, offline local-first open, and schema-safe historical restore. Focused 46/46, full 266/266, migration guard, TypeScript, diff check, and the 284-input/1,051-output build pass. Independent recheck and deployed-preview offline/conflict/keyboard evidence remain open; an atomic server save/idempotency key is still a separately reviewed forward-migration concern.

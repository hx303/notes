...
- P05 editor recovery started from `a4662b1d`. The second checkpoint adds a wired IndexedDB outbox, interrupted-save recovery, Web Locks/BroadcastChannel coordination, immutable cross-tab payloads, an inline three-choice conflict comparison, account-epoch invalidation, offline local-first open, and schema-safe historical restore. Focused 42/42, full 262/262, migration guard, TypeScript, diff check, and the 284-input/1,051-output build pass. Independent recheck and deployed-preview offline/conflict/keyboard evidence remain open; an atomic server save/idempotency key is still a separately reviewed forward-migration concern.

...
## P06 document-link integrity production deployment - 2026-07-22

- Completion time: 2026-07-22 07:17 CST.
- Executor and authorization: Codex root agent; after the earlier explicitly authorized attempt stopped before any write, the site owner requested a production migration retry. The authorized scope remained backup, preflight, deployment, and verification of only `20260721000100_document_links_integrity.sql`.
- Git source: PR #27 branch head `f3c14799c3619fe517bb9cb412c90fa784aadf99`; migration SHA-256 `369D3EE97A0679766BDBA875CE45C5B659569D2D8DD36C222622BEE94C75054D`; Supabase CLI `2.109.1`.
- Target: linked Supabase project `agocyybolrisqujvjqdj`, reported `ACTIVE_HEALTHY`; no Edge Function, Secret, AI setting, account setting, publication, or business-content change was authorized or performed.
- Pre-change backups outside the worktree: `work/backups/supabase-production-20260722-document-links/public-schema.sql`, 93,458 bytes, SHA-256 `DECA8BE027850562156CA22B32D6E61645D66EBBD3AC89370D57004EA2D34ED2`; `public-data.sql`, 3,672,430 bytes, SHA-256 `DC9D1C8241B1F1C2934C70C26B3D0CFA61794D6072EF8E35C0A8CD641F70609C`; `migration-ledger-data.sql`, 111,835 bytes, SHA-256 `18334202671A242D76000BCA65F90FDF11D004076819476B57B2AB49A2F5276A`.
- Preflight ledger: exactly 17 rows and 17 unique versions; `20260721000100` was absent. Linked `migration list` and `db push --dry-run` showed only `20260721000100_document_links_integrity.sql` pending.
- Preflight data and activity: `documents=264`, soft-deleted documents `0`, `document_links=155` (`prerequisite=3`, `related=152`), missing-endpoint links `0`, cross-owner links `0`, cross-knowledge-base links `0`, soft-deleted-endpoint links `0`, long transactions `0`, and waiting locks `0`.
- Production write: the only committed production write was an atomic linked `db push --yes`; it applied only `20260721000100_document_links_integrity.sql`. No manual SQL, migration repair, unrelated migration, content mutation, Secret/config change, or model request ran.
- Postflight ledger: exactly 18 rows and 18 unique versions; `20260721000100` exists exactly once; linked migration list is fully aligned and `db push --dry-run` reports the remote database up to date with zero pending migrations.
- Integrity and ACL verification: the endpoint-validation trigger exists once and is enabled; its function exists once, is `SECURITY DEFINER`, uses `search_path=pg_catalog, pg_temp`, and has zero EXECUTE grants to `PUBLIC`, `anon`, `authenticated`, or `service_role`. RLS is enabled with exactly four authenticated owner policies for SELECT, INSERT, UPDATE, and DELETE. `anon` has none of those table privileges; `authenticated` retains all four.
- Postflight data invariants: document and link counts, relation distribution, and all missing/cross-owner/cross-knowledge-base/soft-deleted-endpoint counts matched preflight exactly. There were no long transactions or waiting locks.
- Transactional negative verification: an invalid owner/link insert was attempted inside an explicit transaction, the new trigger returned the exact expected `check_violation`, the transaction was rolled back, and the verification marker count was `0` afterward.
- Post-change schema snapshot: `work/backups/supabase-production-20260722-document-links/public-schema-post.sql`, 96,127 bytes, SHA-256 `8C85B7934F4ECB2A3AE54A10D46E1C9E1E55AD6E81B35FAC9B89E3485AFC2685`.
- Error or discrepancy: the first read-only ledger retry again failed before backup with `LegacyDbConfigLoginRoleNetworkError`. A separate read-only Management API check then succeeded, the linked ledger retry recovered, and all gates were rerun from the start. Docker Desktop was initially stopped and was started solely for the logical backups. No production write occurred before every gate passed.
- Recovery boundary: the new constraints are fail-closed and should not be reversed by default. Any defect must use a separately reviewed forward migration; the protected pre-change backups are retained only for separately authorized, precisely scoped recovery.
- Final status: `20260721000100` is deployed and verified in production, existing data is unchanged, the integrity contract is active, and the production migration list has zero pending entries.

## P1A site-owner role invariant production deployment - 2026-07-26

- Operation window: 2026-07-26 09:31:48-09:33:12 CST (01:31:48-01:33:12 UTC).
- Executor and authorization: Codex root agent; the site owner explicitly authorized pushing the deterministic-gate branch, creating PR #35, converting it to Ready, merging it, and then rerunning production backup, preflight, deployment of only `20260722000100_site_owner_role_invariant.sql`, and postflight from the latest `main` and a fresh evidence directory.
- Git source: PR #35 merged as `600bfa5656c59a47e299c1254e45e4be5af8ccca`; migration SHA-256 `C38D6DA0405298E3E973DA733EFC992E71E735F0EB7F59E2A1BE3CCB516E10C3`; generated runbook script SHA-256 `58DFD42DAFB92A33B96B719DCBF9B3B2A35D03DB5D8C5D979FAA15E86A3DEF83`; Supabase CLI `2.109.1`.
- Target: linked Supabase project `agocyybolrisqujvjqdj`; the only authorized production migration was `20260722000100`. No Edge Function, Secret, AI/account setting, publication, or business-content change was performed.
- Pre-change backups outside the repository: `public-schema.sql`, 96,127 bytes, SHA-256 `8C85B7934F4ECB2A3AE54A10D46E1C9E1E55AD6E81B35FAC9B89E3485AFC2685`; `public-data.sql`, 3,828,139 bytes, SHA-256 `895DF2E5BA393FF0E4F6BD2A059E5466417B7E1E923DD7CC7BF692A55E03BBD9`; `migration-ledger-data.sql`, 118,063 bytes, SHA-256 `C88B89BE8CC2F30961949973E21A1796E1049A0FDC1730E45DD4FD068FC03E0D`. All three files were nonempty and their independently recomputed hashes matched.
- Evidence location: protected outside-repository directory `work/production-backups/p1a-20260726T013148Z`; 34/34 required evidence artifacts were present and nonempty. Raw role fingerprints and credentials were not copied into this record.
- Preflight: the migration ledger was exactly 18/18 and aligned; `20260722000100` was absent remotely and was the only pending migration. Contract checks for the function, ACLs, RLS, policies, and protected site-owner state passed. The initial and immediate protected-owner fingerprints matched, and the activity gates reported no blocking long transaction or waiting lock condition.
- Production write: the only production write command was one atomic linked `db push --yes`; it applied only `20260722000100_site_owner_role_invariant.sql`. No `--include-all`, migration repair, Dashboard SQL, manual SQL, or second push was used.
- Postflight ledger: exactly 19 rows and 19 unique versions; `20260722000100` exists exactly once; local and remote migration lists are aligned; linked `db push --dry-run` reports the remote database up to date with zero pending migrations.
- Security verification: the hardened `grant_role` contract rejects site-owner role mutation with SQLSTATE `42501`; `anon` and `PUBLIC` remain denied while the required `authenticated` and `service_role` access is preserved. The `is_site_owner` function, table ACLs, RLS policies, and protected-owner state remained within the reviewed contract. Initial, immediate, and postflight protected-owner fingerprints matched, and all three activity gates passed.
- Prior stopped attempts: three fresh evidence runs stopped before `db push` because of a runner-scope error, a local object-rendering issue, and a raw-order comparison false positive. Their evidence was retained without reuse. PR #35 added deterministic comparison coverage; the successful operation then restarted from latest `main` and a new evidence directory.
- Recovery boundary: do not destructively roll back this fail-closed invariant. Any defect must use a separately reviewed forward migration; retain the protected backups for separately authorized, precisely scoped recovery.
- Final status: PR #35 merged with green deployment checks; production migration `20260722000100` succeeded; all preflight and postflight gates passed; no business data, AI configuration, or model call changed.

## P1B atomic document snapshot save production deployment - 2026-08-03

- Operation window: 2026-08-03 11:00:39-11:02:17 CST (03:00:39-03:02:17 UTC), followed by an independent read-only postflight recheck.
- Executor and authorization: Codex root agent; the site owner explicitly authorized continuing the plan, converting and merging PR #42, and performing any necessary production deployment. The authorized release chain was backup, read-only preflight, deployment of only `20260722000200_atomic_document_snapshot_save.sql`, postflight, and production-record reconciliation.
- Git source: PR #42 merged as `c80c9f77973a2866991810411891327fc2c722d7`; migration SHA-256 `EC5B4C48465C2E6FB1B163D83FC96D2E3A0FCBC5653664FEA098141CB9F7D357`; one-session deployment script SHA-256 `2B2F5464A95D161BB4E09A446023E2A88907421F6963954D33BE0780D25EA8B5`; Supabase CLI `2.109.1`.
- Target and write boundary: linked Supabase project `agocyybolrisqujvjqdj`. The only production write was one linked `db push --yes`, permitted only after the initial and immediate gates proved exactly one pending migration. No `--include-all`, migration repair, Dashboard/manual SQL, second push, Edge Function/Secret/config change, AI call, publication action, or account-content write was performed.
- Protected evidence: fresh outside-repository directory `production-evidence/p1b-20260803T030039Z`. It contains the identity, authorization boundary, initial/immediate/postflight outputs, completion boundary, and three nonempty pre-change backups with matching SHA-256 hashes.
- Pre-change backups: `public-schema.sql`, 96,264 bytes, SHA-256 `72397DBE2ADB8F1342CDB1C8F52215E7157E4139C26C2E9890F5F6A2DD8B527A`; `public-data.sql`, 3,828,095 bytes, SHA-256 `F2CEF72AC51BECE5249BCFD727B651DF013D5A0BA487D5A454308067EF41756A`; `migration-ledger-data.sql`, 132,347 bytes, SHA-256 `2692D1116BECBC7C7D50EA35D3D1F0C0A23EAC723CCB12B663F208F5C74BF820`.
- Initial and immediate gates: Git SHA, clean worktree, linked project, and CLI version remained exact. The remote ledger contained exactly 20 unique approved versions through `20260722000150`; both dry-runs named only `20260722000200_atomic_document_snapshot_save.sql`; migration-list, dry-run, activity, preflight, and state outputs matched their approved counterparts. The activity gate, preflight, and state-fingerprint sentinel each appeared exactly once.
- Production write: the single linked push applied only `20260722000200_atomic_document_snapshot_save.sql` and completed successfully. The remote ledger advanced from exactly 20 to exactly 21 entries.
- Postflight contract: `atomic_document_snapshot_contract_passed` appeared exactly once. The exact 21-version ledger and migration name passed; the atomic-save RPC ABI/body/security fingerprint passed; EXECUTE remains restricted to `authenticated` plus the owner and denied to `anon` and `service_role`; the private schema, helpers, receipt table, trigger, constraints, RLS, and PostgREST non-exposure contracts passed; the tag/knowledge-base composite-owner invariant remained valid.
- Business-state invariant: the fingerprint over knowledge bases, documents, document versions, tags, document-tag relations, document links, document sources, and publication snapshots remained byte-identical at `e5239f04ffda42d817c6bebe1538eced`, proving that the deployment did not mutate represented business rows.
- Postflight ledger and activity: the migration list contained exactly 21 aligned local/remote versions with `20260722000200` exactly once; the linked dry-run reported the remote database up to date with zero pending migrations; the postflight activity sentinel appeared exactly once.
- Independent verification: after the guarded session completed, a separate read-only migration list, zero-pending dry-run, complete contract, state fingerprint, and activity gate all passed again against production.
- Recovery boundary: do not destructively roll back the atomic-save objects or ledger row. Any defect must use a separately reviewed forward migration; retain the three protected backups for a separately authorized, precisely scoped recovery.
- Final status: PR #42 is merged, `20260722000200` is deployed and verified, the production ledger is exactly 21/21 with zero pending migrations, represented business data is unchanged, and no AI or unrelated production state changed.

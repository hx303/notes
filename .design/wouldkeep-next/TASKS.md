...
- [x] **A20 — AI 用量与预算保护**：为成功/失败调用写审计，实施并发、日限额、月预算和站长 feature flag；真实密钥只在 Function Secret。 _Completed 2026-07-17: production Supabase authority, service-role RPC, versioned rate card, RLS/failure evidence, configuration, Secrets, and `ai-write` v4 are deployed with both live gates off; no production paid call has run._

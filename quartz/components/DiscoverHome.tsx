...
const DiscoverHome: QuartzComponent = ({ fileData, allFiles }: QuartzComponentProps) => {
  const notes = allFiles
    .filter((file) => file.slug && file.slug !== "index" && file.frontmatter?.title && file.dates?.modified)
    .sort((a, b) => (b.dates!.modified?.getTime() ?? 0) - (a.dates!.modified?.getTime() ?? 0))
  const featured = notes.slice(0, 4)
  const total = notes.length
  const topics = new Set(notes.map((file) => file.knowledgeMetadata?.primaryTopic).filter(Boolean)).size
  ...
  return (
    <div class="discover-home" data-discover-home>
      <section class="discover-hero">
        ...
        <p class="discover-lede">这里不是文件夹的镜像，而是一座会持续生长的知识花园：从一个问题出发，沿着主题、路径和他人的连接，找到下一步。</p>
        <div class="discover-actions">
          <a class="discover-primary" href={href("paths/index")}>沿一条路径开始</a>
          <a class="discover-secondary" href={href("map/index")}>打开知识地图</a>
        </div>
      </section>
        ...
        <ol class="discover-records">
          {featured.map((file, index) => (
            <li>
              ...
              <time dateTime={file.dates!.modified!.toISOString()}>{file.dates!.modified!.toLocaleDateString("zh-CN")}</time>
            </li>
          ))}
        </ol>
      </section>
      ...
      <section class="discover-stats" aria-label="知识库概览">
        <div><strong>{total}</strong><span>公开记录</span></div>
        <div><strong>{topics}</strong><span>正在交汇的主题</span></div>
        <div><strong>3</strong><span>可继续的学习路径</span></div>
      </section>
    </div>
  )
}
